...
class Dataset_Cut_8(data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        for i in range(4):
            for j in range(i, 4):
                if i != j:
                    data_fcn[n_dim, :data_len, :data_len] = np.matmul(data_seq[:data_len, i].reshape(-1, 1), data_seq[:data_len, j].reshape(1, -1)) + np.matmul(data_seq[:data_len, j].reshape(-1, 1), data_seq[:data_len, i].reshape(1, -1))
                else:
                    data_fcn[n_dim, :data_len, :data_len] = np.matmul(data_seq[:data_len, i].reshape(-1, 1), data_seq[:data_len, j].reshape(1, -1))
                n_dim += 1
        '''
        for n, cord in enumerate(perm):
            i, j = cord
...
class Dataset_Cut_concat_new_merge(data.Dataset):
  ...
  def merge_data(self):
        ...
        self.data2.seq_length = np.concatenate((self.data1.seq_length,self.data2.seq_length,self.data3.seq_length),axis=0)
        self.data2.pairs = np.concatenate((self.data1.pairs,self.data2.pairs,self.data3.pairs),axis=0)
        self.data2.data_name = np.concatenate((self.data1.data_name,self.data2.data_name,self.data3.data_name),axis=0)
...
def creatmat(data, device=None):
    ...
    with torch.no_grad():
        idx_to_base = ['A', 'U', 'C', 'G']
        data = [idx_to_base[np.argmax(vec)] for vec in data]
